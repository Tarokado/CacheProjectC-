...
void PrintL2COM(){
	FILE *readHistory = fopen(ComFile, "r");
	char message[100];
	if(mode != 0) {
		printf("\n\t\t\t  L2 COMMUNICATION: ");
...
void InsEvict(int set_index, int evict_tag) { //Identify data within a set
// Loop through all lines in the set, 
	for(int line_index = 0; line_index < INS_WAY; line_index++) { //line_index iterates over all line, INS_WAY represent the way for the set(the line in 1 set)
		// Data_Cache[set_index][line_index].lru = -1; // default LRU value when empty
		if(Ins_Cache[set_index][line_index].tag == evict_tag) { //line_index => identified by set_index. If match evict_tag => match found => proceed to handle eviction
...
void InsHit(int set_index, int line_index, int n) { //represents the types of operation triggered the cache(instruction fetch)
...
void InsRead(int set_index, int req_tag, int n) {
	int empty_line = -1;
	for(int i = INS_WAY-1; i >= 0; i--) { // find suitable line_index
		if(Ins_Cache[set_index][i].state == 'I') 
			empty_line = i; // store least line_index empty line when no hit
		else { // check valid
			if(Ins_Cache[set_index][i].tag == req_tag) { // a hit
				// Dont need to change address since a hit
	...
	if(empty_line >= 0) {
		...
		// Line was fulled -> evict LRU line
		for(int i = 0; i < INS_WAY; i++) { // find suitable line_index
			if(Ins_Cache[set_index][i].lru == 0) { // if this is LRU line_index
